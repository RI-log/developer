...
cron登録
```bash
crontab /data/archive/00_script/DenyExportCron.cron
```
...
DenyExportCron.cron
```bash
0 6 * * * /usr/bin/nice -n 15 /usr/bin/ionice -c 3 /usr/bin/python3 /data/archive/00_script/DenyExportCron.py >> /data/archive/01_csv/DenyExport.log 2>&1
```
...
cron登録解除
```bash
crontab -l | grep -vF '/data/archive/00_script/DenyExportCron.py' | crontab -
```
